...
const Questions = (props) => {
  ...
  const { quess, getQuess, editQuess, Age } = context;
...
const getlatestques = (value)=>{
  ...
  const updateQues = (currentQues) => {
    ...
    console.log(currentQues.options[0] + "test");
  };

  const handleClick = (e) => {
    console.log("updating note...", ques.id);
    editQuess(
      ques.id,
      ques.equestion,
      ques.eoption1,
  ...
  for (let i = 0; i < Queslength; i++) {
    QueslengthList.push(<li key={i+1}> <button onclick={getlatestques(i+1)}>{i+1}</button></li>);
  }
